...
export function HowItWorks() {
  const { howItWorks } = therapyPracticeWebsiteContent;
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        <div className="mt-10 grid gap-6 md:mt-14 md:grid-cols-3 md:gap-10">
          {howItWorks.steps.map((step, index) => (
            <article
              key={step.step}
              className="group relative flex flex-col rounded-3xl border border-border/45 bg-card/72 p-6 transition-all duration-500 hover:-translate-y-1 hover:shadow-lg"
            >
              {index < howItWorks.steps.length - 1 && (
                <div className="absolute -right-8 top-1/2 hidden h-px w-6 -translate-y-1/2 bg-border/60 md:block" aria-hidden="true" />
              )}

              <span className="font-serif text-5xl font-bold text-primary/70 transition-colors duration-500 group-hover:text-primary md:text-7xl">
                {step.step}
              </span>
