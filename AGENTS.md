...
- Homepage now includes a small collection-backed `Announcements` section between `Services` and `Contact` for workshops, group launches, and short notices.
...
- Announcements are backed by a separate Astro content collection in `src/data/announcements.json` and the homepage only renders the first two published entries.
...
- Post-domain launch checklist: `POST_DOMAIN_LAUNCH_CHECKLIST.md`
...
- Announcements section: `src/components/sections/Announcements.tsx`
...
- Announcements collection helper: `src/content/announcements.ts`
- Services collection helper: `src/content/services.ts`
- Services data entries: `src/data/services.json`
- Announcements data entries: `src/data/announcements.json`
...
- Replace placeholder contact data if real client contact details become available:
  - `email@example.com`
  - `+30 XXX XXX XXXX`
- Once the final domain is ready, work through `POST_DOMAIN_LAUNCH_CHECKLIST.md`.
- If a CMS is introduced later, keep the `services` collection schema stable and swap the loader before changing `Services.tsx`.
- Keep the `announcements` collection small and current; the homepage is designed for at most two visible announcements at a time.
